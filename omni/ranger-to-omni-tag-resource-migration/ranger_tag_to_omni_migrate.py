...
def convert_tag_definition_to_mds_format(tag_def: Dict[str, Any], tag_type_override: Optional[str] = None) -> Dict[str, Any]:
    ...
    # MDS source should always be "Portal" as per requirement
    source = 'Portal'
...
def send_resources_to_mds(
    ...
    Args:
        mds_url: Base URL of the MDS server (e.g., http://localhost:6080/omni-metadata)
        app_code: Application code for the resources
        service_name: Name of the service
...
def build_tag_resource_mappings(
    ...
    for idx, (service_resource, resource_path) in enumerate(zip(service_resources, resource_paths)):
        ...
        for tag_id in tag_ids:
            ...
            # Get tag name from tag definition or tag object
            tag_name = None
            if str(tag_id) in tag_definitions:
                tag_name = tag_definitions[str(tag_id)].get('name')
            elif tag_id in tag_definitions:
                tag_name = tag_definitions[tag_id].get('name')
            
            # Fallback: use tag.type if tag name not found in definitions
            if not tag_name:
...
Examples:
  ...
  python3 get_tagged_resources.py --admin-url http://localhost:6080 --service-name privacera_snowflake --sync-resources-to-mds --app-code APP001 --service-type SNOWFLAKE
  python3 get_tagged_resources.py --admin-url http://localhost:6080 --service-name privacera_snowflake --sync-tag-resource-mappings --service-type SNOWFLAKE --app-code APP001
        """
    )
    ...
    parser.add_argument(
        '--app-code',
        required=True,
        help='Application code for resource sync and tag-resource mappings'
    )
    ...
    try:
        ...
        print(f"App Code: {args.app_code}")
        ...
        if args.sync_tags_to_mds or args.sync_resources_to_mds or args.sync_tag_resource_mappings:
            ...
            if args.sync_tags_to_mds and converted_tag_definitions:
                ...
                if success_count > 0:
                    print(f"Successfully synced {success_count} tag def(s) to MDS")
                
                if error_count > 0:
                    print(f"⚠️  Warning: {error_count} tag definition(s) failed to sync")
            ...
            if args.sync_tag_resource_mappings and mappings:
                try:
                    ...
                    for batch_idx, mapping_batch in enumerate(mapping_batches):
                        ...
                        total_synced += len(mapping_batch)
                    
                    print(f"Successfully synced {total_synced} tag-resource mapping(s) to MDS in {len(mapping_batches)} batch(es)")
